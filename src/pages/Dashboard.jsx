...
import { Package, ClipboardList, AlertCircle, Plus, ChevronRight, ClipboardCheck, Eye } from 'lucide-react';
...
const Dashboard = () => {
   ...
   const canRequest = role === 'farmasi';
   const canVerify = role === 'logistik';
   const canMonitor = role === 'logistik' || role === 'manajemen';
         ...
         <div className="px-6 flex space-x-6 overflow-x-auto no-scrollbar border-b border-gray-100">
            {tabs.map(tab => (
               <button
                  ...
                  className={`pb-3 text-sm font-semibold transition-all relative whitespace-nowrap ${activeTab === tab ? 'text-primary' : 'text-gray-400'
                     }`}
               >
                  {tab}
            ...
            <div className="grid grid-cols-2 gap-3">
               {canRequest && (
                  <button
                     ...
                     className="text-left bg-white p-4 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all"
                  >
                     <div className="w-10 h-10 rounded-2xl bg-primary/10 flex items-center justify-center mb-3">
                        <Plus className="w-5 h-5 text-primary" />
                     </div>
                     <div className="text-sm font-extrabold text-gray-900">Input Permintaan</div>
                     <div className="text-[10px] text-gray-400 font-medium mt-1">Khusus Farmasi</div>
                  </button>
               )}
               {canVerify && (
                  <button
                  ...
                  >
                     ...
                     <div className="text-sm font-extrabold text-gray-900">Verifikasi</div>
                     <div className="text-[10px] text-gray-400 font-medium mt-1">Khusus Logistik</div>
                  </button>
               )}
               {canMonitor && (
                  <button
                  ...
                  >
                     ...
                     <div className="text-sm font-extrabold text-gray-900">Monitoring</div>
                     <div className="text-[10px] text-gray-400 font-medium mt-1">Manajemen / Logistik</div>
                  </button>
               )}
            </div>
         </div>
                  ...
                  <tbody className="text-xs font-medium text-gray-700">
                     {requests.map((req, idx) => (
                        <tr key={req.id} className="border-t border-gray-50">
                           ...
                           <td className="py-4">
                              <span className={`px-2 py-1 rounded-lg ${req.status === 'Pending' ? 'text-yellow-600 bg-yellow-50' :
