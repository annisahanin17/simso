    ...
    <div className="min-h-screen bg-gray-50 max-w-md mx-auto shadow-2xl overflow-hidden border-x border-gray-100 flex flex-col">
      ...
      <div className="p-6 bg-white border-b border-gray-100 flex space-x-2 overflow-x-auto no-scrollbar">
         {['Semua', 'Pending', 'Diproses', 'Selesai'].map(f => (
           <button key={f} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap border transition-all ${
             f === 'Semua' ? 'bg-primary text-white border-primary shadow-lg shadow-primary/30' : 'bg-white text-gray-500 border-gray-200'
           }`}>
             {f}
           </button>
         ))}
          ...
          requests.map(req => (
            <div key={req.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 space-y-3">
               ...
               <div className="pt-2 border-t border-gray-50 flex justify-between items-center">
                  ...
                  <button className="text-primary text-[10px] font-bold flex items-center">
                    Detail <RefreshCw className="w-3 h-3 ml-1 rotate-90" />
                  </button>
               </div>
            </div>
          ))
